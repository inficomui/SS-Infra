...
export default function DriverAddMaintenanceScreen() {
    ...
    useEffect(() => {
        if (machinesData?.machines && userData?.user?.assignedVehicle) {
            const assigned = userData.user.assignedVehicle.toLowerCase();
            const found = machinesData.machines.find(m => 
                m.name.toLowerCase().includes(assigned) || 
                m.registration_number.toLowerCase().includes(assigned)
            );
            if (found) setMachineId(found.id.toString());
        }
    }, [machinesData, userData]);
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.machineScroll}>
                    {machinesData?.machines?.map((m) => (
                        <TouchableOpacity
                            key={m.id}
                            style={[styles.machineChip, { backgroundColor: machineId === m.id.toString() ? colors.primary : colors.card, borderColor: machineId === m.id.toString() ? colors.primary : colors.border }]}
                            onPress={() => setMachineId(m.id.toString())}
                        >
                            <MaterialCommunityIcons name="crane" size={20} color={machineId === m.id.toString() ? '#FFF' : colors.textMuted} />
                            <Text style={{ color: machineId === m.id.toString() ? '#FFF' : colors.textMain, fontWeight: '700' }}>{m.name}</Text>
                        </TouchableOpacity>
                    ))}
                </ScrollView>
                ...
                <View style={styles.formSection}>
                    ...
                    <TextInput label="Cost (₹)" value={cost} onChangeText={setCost} keyboardType="numeric" mode="outlined" style={styles.input} outlineColor={colors.border} activeOutlineColor={colors.primary} />
                    ...
                    <TouchableOpacity onPress={() => setShowDatePicker(true)} style={[styles.datePickerBtn, { borderColor: colors.border, backgroundColor: colors.card }]}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}><MaterialCommunityIcons name="calendar" size={20} color={colors.primary} /><Text style={{ color: colors.textMain }}>{serviceDate.toLocaleDateString()}</Text></View>
                        <Text style={{ color: colors.primary, fontWeight: '700', fontSize: 12 }}>CHANGE</Text>
                    </TouchableOpacity>

                    <TextInput label="Description" value={description} onChangeText={setDescription} multiline numberOfLines={3} mode="outlined" style={[styles.input, { height: 100 }]} outlineColor={colors.border} activeOutlineColor={colors.primary} />
                </View>

                <View style={styles.formSection}>
                    <Text style={[styles.label, { color: colors.textMuted }]}>Attachments</Text>
                    <View style={styles.imageRow}>
                        <View style={{ flex: 1 }}><Text style={styles.imgLabel}>Service Photo</Text><TouchableOpacity style={[styles.imageUpload, { borderColor: colors.border }]} onPress={() => pickImage('service')}>{serviceImage ? <Image source={{ uri: serviceImage.uri }} style={styles.uploadedImage} /> : <MaterialCommunityIcons name="camera-plus" size={32} color={colors.textMuted} />}</TouchableOpacity></View>
                        <View style={{ flex: 1 }}><Text style={styles.imgLabel}>Invoice / Bill</Text><TouchableOpacity style={[styles.imageUpload, { borderColor: colors.border }]} onPress={() => pickImage('invoice')}>{invoiceImage ? <Image source={{ uri: invoiceImage.uri }} style={styles.uploadedImage} /> : <MaterialCommunityIcons name="file-plus" size={32} color={colors.textMuted} />}</TouchableOpacity></View>
                    </View>
                </View>

                <Button mode="contained" onPress={handleSubmit} loading={isLoading} disabled={isLoading} style={styles.submitBtn} labelStyle={styles.submitBtnLabel}>
                    Submit Record
                </Button>
            </ScrollView>

            {showDatePicker && <DateTimePicker value={serviceDate} mode="date" display="default" onChange={(e, date) => { setShowDatePicker(false); if (date) setServiceDate(date); }} />}
        </View>
    );
}
