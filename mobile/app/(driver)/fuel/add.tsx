...
export default function DriverAddFuelLogScreen() {
    ...
    useEffect(() => {
        if (machinesData?.machines && userData?.user?.assignedVehicle) {
            ...
            if (found) {
                setMachineId(found.id.toString());
            }
        }
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.machineScroll}>
                    {machinesData?.machines?.map((m) => (
                        ...
                        >
                            ...
                            />
                            <Text style={{ color: machineId === m.id.toString() ? '#FFF' : colors.textMain, fontWeight: '600' }}>
                                {m.name}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                <View style={styles.inputGroup}>
                    ...
                    <View style={styles.imageRow}>
                        <TouchableOpacity style={[styles.imageUpload, { borderColor: colors.border }]} onPress={() => pickImage('before')}>
                            {readingBefore ? <Image source={{ uri: readingBefore.uri }} style={styles.uploadedImage} /> : (
                        ...
                        <TouchableOpacity style={[styles.imageUpload, { borderColor: colors.border }]} onPress={() => pickImage('after')}>
                            {readingAfter ? <Image source={{ uri: readingAfter.uri }} style={styles.uploadedImage} /> : (
                                <View style={{ alignItems: 'center' }}>
                                    <MaterialCommunityIcons name="camera-plus" size={24} color={colors.textMuted} />
                ...
                <Button
                    ...
                    loading={isLoading}
                    disabled={isLoading}
                    style={{ marginTop: 32, borderRadius: 12, paddingVertical: 6 }}
