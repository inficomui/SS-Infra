...
import { Text, ActivityIndicator, Searchbar, IconButton, Menu, Divider, Surface } from 'react-native-paper';
...
export default function OwnerFuelLogsScreen() {
    ...
    const [searchQuery, setSearchQuery] = useState((operatorName as string) || '');
    ...
    useEffect(() => {
        if (operatorName) {
            setSearchQuery(operatorName as string);
        }
    }, [operatorName]);
    ...
    const processedLogs = useMemo(() => {
        ...
        // Search filter (Machine name or Operator name)
        if (searchQuery) {
            const query = searchQuery.toLowerCase();
            list = list.filter(log =>
                log.machine?.name.toLowerCase().includes(query) ||
                log.operator?.name.toLowerCase().includes(query)
            );
        }
        ...
        >
            <View style={styles.cardHeader}>
                ...
                <View style={{ alignItems: 'flex-end' }}>
                    ...
                    <Text style={[styles.operatorSubText, { color: colors.textMuted }]}>{log.operator?.name || t('fuel_management.unknown')}</Text>
                </View>
            </View>
                        ...
                        anchor={
                            ...
                            >
                                ...
                                <Text style={[styles.filterText, { color: colors.textMain }]}>
                                    {sortBy === 'newest' ? t('fuel_management.newest') : t('fuel_management.oldest')}
                                </Text>
                            </TouchableOpacity>
                        }
                    >
